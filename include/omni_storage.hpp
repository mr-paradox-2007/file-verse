...
#define BLOCK_SIZE 65536  // 64KB blocks
...
struct MetadataEntry {
    ...
    uint32_t parent_index;      // Parent directory entry index
    char name[32];              // Short filename
    uint32_t start_block;       // First block index (0=empty)
    uint64_t total_size;        // Logical file size
    uint32_t owner_id;          // User ID who owns this
...
struct BlockHeader {
    uint32_t next_block;        // Next block index (0=end)
...
private:
    ...
    std::vector<uint8_t> block_bitmap;  // 0=free, 1=used
